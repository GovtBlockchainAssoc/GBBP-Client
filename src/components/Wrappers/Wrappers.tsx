...
// styles
var useStyles = makeStyles(theme => ({ badge: { fontWeight: 600, height: 16, minWidth: 16, }, }));
...
function Badge({ children, colorBrightness, color, ...props }) {
    ...
    return (
        <Styled>
            {styledProps => (
                <BadgeBase classes={{ badge: classnames(classes.badge, styledProps.classes.badge), }} {...props} >
                    {children}
                </BadgeBase>
            )}
        </Styled>
...
function Button({ children, color, className, ...props }) {
    ...
    return (
        <Styled>
            {({ classes }) => (
                <ButtonBase
                    classes={{ contained: classes.contained, root: classes.root, outlined: classes.outlined, }}
                    {...props}
                    className={classnames( { [classes.select]: props.select, }, className, )}
                >
                    {children}
                </ButtonBase>
            )}
...
function getColor(color, theme, brightness = "main") {
    if (typeof (brightness) != "string") brightness = "main";
...
function getFontSize(size, variant = "", theme) {
    ...
    switch (size) {
        case "xsm": multiplier = 0.4; break;
        case "sm": multiplier = 0.8; break;
        case "md": multiplier = 1.5; break;
        case "xl": multiplier = 2; break;
        case "xxl": multiplier = 3; break;
