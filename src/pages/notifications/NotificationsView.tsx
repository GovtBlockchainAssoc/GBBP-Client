    ...
    <React.Fragment>
        ...
        <Grid container>
            <ToastContainer className={classes.toastsContainer} progressClassName={classes.notificationProgress}
                closeButton={<CloseButton closeToast className={classes.notificationCloseButton} />} closeOnClick={false} />
            <Grid item xs={12} md={6} lg={4}>
                <Widget title="Layout Options" disableWidgetMenu noBodyPadding bodyClass header>
                    <Typography weight size colorBrightness color>
                        There are few position options available for notifications. Click any of them to change notifications position:</Typography>
            ...
            <Grid item xs={12} md={6} lg={4}>
                <Widget title="Notifications Types" disableWidgetMenu noBodyPadding bodyClass header>
                    ...
                    <div className={classes.buttonsContainer}>
                        <Button color variant="contained" colortheme="primary" onClick={() => props.handleNotificationCall('info')} className={classnames(classes.notificationCallButton)}>Info Message</Button>
                        <Button color variant="contained" colortheme="warning" onClick={() => props.handleNotificationCall('error')} className={classnames(classes.notificationCallButton)}>Error + Retry Message</Button>
                        <Button color variant="contained" colortheme="success" onClick={() => props.handleNotificationCall('success')} className={classnames(classes.notificationCallButton)}>Success Message</Button>
                    </div>
                </Widget>
            </Grid>
            <Grid item xs={12} md={6} lg={4}>
                <Widget title="Usage" disableWidgetMenu noBodyPadding bodyClass header>
                    <Typography weight size colorBrightness color>
                        Notifications are created with the help of <a href="https://github.com/fkhadra/react-toastify">react-toastify</a></Typography>
                    <div className={classes.codeContainer}>
                        ...
                        `}</SyntaxHighlighter>
                        <Typography variant="caption" weight size colorBrightness color>For more API information refer to the library documentation</Typography>
                    </div>
                </Widget>
            </Grid>
            <Grid item xs={12} md={6} lg={4}>
            ...
            <Grid item xs={12} md={6} lg={4}>
                <Widget title="Notification Types Examples" disableWidgetMenu noBodyPadding bodyClass header>
                    <NotificationCustomComponent variant className={classes.notificationItem} type="report" message="New report has been received" color="secondary" />
                    <NotificationCustomComponent variant className={classes.notificationItem} type="feedback" message="New user feedback received" color="primary" />
                    <NotificationCustomComponent variant className={classes.notificationItem} type="shipped" message="The item was shipped" color="success" />
                    <NotificationCustomComponent variant className={classes.notificationItem} type="message" message="The new message from user @nahawaii" color="warning" />
                    <NotificationCustomComponent variant className={classes.notificationItem} type="upload" message="Your file is ready to upload" color="primary" />
                    <NotificationCustomComponent variant className={classes.notificationItem} type="disc" message="The disc is full" color="info" />
                </Widget>
            </Grid>
            <Grid item xs={12} md={6} lg={4}>
                <Widget title="Notification Types Examples" disableWidgetMenu noBodyPadding bodyClass header>
...
const styles = (theme) => ({
    ...
    layoutText: { color: tinycolor(theme.palette.background.light).darken().toHexString(), },
    layoutButtonsRow: {
        width: '100%',
        display: 'flex',
        justifyContent: 'space-between',
    ...
    layoutButtonActive: { backgroundColor: tinycolor(theme.palette.background.light).darken().toHexString(), },
    buttonsContainer: {
        display: 'flex',
        // flexDirection: 'column',
